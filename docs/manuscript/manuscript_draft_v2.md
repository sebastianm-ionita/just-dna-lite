...
We present Just-DNA-Lite, an open-source modular platform for personal genome analysis designed for research and educational use, combining two capabilities: variant annotation against curated variant-trait databases, and polygenic risk score (PRS) computation. Just-DNA-Lite runs entirely on a personal computer, processes a whole-genome VCF in under 40 seconds, and produces annotated reports across user-selected modules. The platform is built on a no-code module system: annotation modules consist of a YAML specification and a variant table (CSV or Parquet), with no programming required. Modules are auto-discovered from any fsspec-compatible source (HuggingFace, GitHub, S3, or HTTP), enabling a growing ecosystem of community-contributed annotations.

Just-DNA-Lite is designed to be used in two complementary ways: as a self-contained local web application, or as a set of well-benchmarked genomic tools exposed to AI coding assistants through the Model Context Protocol (MCP) and agent skills. This second mode addresses a fast-growing pattern: researchers and individuals increasingly use large language model assistants such as Claude to explore genomes (public genomes for researchers, their own for individuals), but when an assistant improvises analysis code from scratch the results are frequently unreliable. By packaging validated, benchmarked operations as MCP servers and skills, Just-DNA-Lite lets these users obtain results from validated, benchmarked tools within the agentic environments they already work in (Claude Code, Cursor, Codex, Antigravity, and others), with no interface to learn and no additional API cost beyond the assistant subscription they already hold.

AI fits into the platform in a second way as well: beyond letting assistants run analyses on a genome, it can also generate the platform's own annotation content. A key contribution here is an AI-assisted module creation system built on the Agno agentic framework. In simple mode, a single agent reads a research paper, queries biomedical databases, and produces a validated annotation module in approximately two minutes. In research team mode, a coordinated swarm of agents (Principal Investigator, up to three Researcher agents using different large language models, and a Reviewer with web search access) independently extract and cross-validate variant data, producing consensus-based modules in seven to eight minutes. Both modes output a deterministic specification that can be registered with one click.

The platform ships with expert-curated default modules covering several trait domains (longevity-associated variants, cardiovascular-related variants, lipid metabolism, VO2 max, and athletic performance) as a starting point, alongside over 5,000 polygenic risk scores from the PGS Catalog computed via the just-prs library with Pearson r = 0.9999 concordance against PLINK2 and 5.7--12.3x speedup. The just-prs engine that computes these scores is the first benchmarked toolbox delivered in this agentic mode: released as a standalone package usable as a CLI, a Python library, or through the MCP server and agent skill described above, so that normalizing a VCF, searching the catalogue, computing scores, rendering explanatory charts, and obtaining a plain-language interpretation are available identically in the web app and inside a single agent conversation. Because manual curation cannot keep pace with the volume of new genetic discoveries, the platform is designed around AI-aided module creation and community extension: users and researchers can generate new annotation modules from published papers within minutes and share them through any fsspec-compatible repository, so the annotation catalogue grows with the literature rather than waiting for a central team to update it.

All data stays local. The platform is open-source (AGPL v3), GDPR-friendly by design, and aligned with the principle that individuals have the right to explore their own genomic data. The platform itself does not perform clinical interpretation or generate diagnoses, and is not intended for medical decision-making; an optional, user-initiated AI explanation of results is general and non-prescriptive, not clinical advice. It is a research and educational tool, not a clinical pipeline.
...
1. **VCF ingestion.** The user provides a VCF or VCF.gz file through the web interface. This is the annotated variant file users typically already have — supplied by a sequencing provider, downloaded from a public genome repository, or produced by their own (or any other) bioinformatic pipeline; Just-DNA-Lite starts from this VCF and does not itself perform sequencing, alignment, or variant calling. The file is read using polars-bio, which provides a Polars-native VCF reader.
...
The variant table may be authored as CSV for convenience, but the platform also accepts it as Parquet, and on registration a CSV is compiled to a columnar `weights.parquet` for fast loading; most of the modules we ship are distributed as Parquet for speed. Optional files include `studies.csv` (linking variants to PubMed IDs), `MODULE.md` (human-readable documentation), and `logo.png` (thumbnail image). No Python code is required from the module author. The annotation engine loads modules via Polars LazyFrames; no code from the module is executed.
...
![Just-DNA-Lite web interface: VCF upload, normalized VCF table preview, uploaded-samples list, analysis results and reports, and annotation-module selection](/images/just_dna_lite_annotations.jpg)
...
Consistent with the "runs on a laptop" principle, Just-DNA-Lite prioritizes memory efficiency over raw throughput: peak RAM stays low enough for a personal computer, and warm-run annotation of a whole genome completes in tens of seconds. The detailed speed and memory benchmarks — including the comparison against the Generation I OakVar-based system — are presented in Section 5.1, so they are not repeated here.
...
![AI Module Creator, step 1: open the Module Manager, attach a research-paper PDF, describe the desired module, and click Send to start the multi-agent team](/images/just_dna_lite_AI_module_builder_step_I.jpg)
...
![AI Module Creator, step 2: the module is created automatically (~7 minutes), auto-loaded into the editing slot with all files generated, the AI summarizing what it found, and ready to register as a source](/images/just_dna_lite_AI_module_builder_step_II.jpg)
...
To illustrate the system in practice, we present three scenarios that were executed end-to-end with real biomedical literature. In these three illustrative runs the generated modules required no manual corrections; this is an anecdotal demonstration across three cases, not a systematic accuracy evaluation (see Section 5.3).
...
These default modules originate in the first generation of the platform, the OakVar-based Just-DNA-Seq, described in a separate preprint (Kulaga et al., 2024, [arXiv:2403.19087](https://arxiv.org/abs/2403.19087)). That system was built around longevity and longevity-associated traits, which is why the default set centres on that domain; for the second generation we ported most of the modules largely unchanged, to serve as worked examples of the no-code module format rather than as a definitive trait catalogue. Figure 4 summarises how the curated modules map onto the human lifespan, grouping them by the life stages and risks they concern.

![Just-DNA-Seq modules mapped onto the human lifespan: each curated module (longevity drugs, longevity variants and PRS, neurodegenerative, cancer, cardiovascular/lipid/thrombophilia, and pathogenic-mutation reports) positioned against the death-probability-versus-age curve](/images/longevity_risks.jpg)

**Figure 4. The curated default modules across the human lifespan.** The expert-curated modules (developed by Olga Borysova for the first-generation Just-DNA-Seq) are organised by the life stages and risk categories they address, from early/premature-death risks (pathogenic mutations, hereditary cardiac disease) through mid-life disease risks (cardiovascular, lipid, thrombophilia, cancer) to pro-longevity variants and longevity-drug pharmacogenetics at exceptional ages. The figure illustrates the scope of the default module set; it is not a claim about individual longevity prediction.
...
[FIGURE 5 — pending: Just-DNA-Lite UI screenshot of the Longevity Variants report interface (pathway categorization, SNP table, coloured weights). A suitable screenshot still needs to be added to `images/`.]

### 4.2 Polygenic Risk Scores

Polygenic risk scores (PRS) aggregate the effects of many common variants into a single weighted sum, providing a population-relative measure for a given trait. PRS computation is a standard bioinformatics operation used extensively in research (Lambert et al., 2021). We developed the just-prs library ([github.com/dna-seq/just-prs](https://github.com/dna-seq/just-prs)) as the PRS computation engine for Just-DNA-Lite. Although it powers PRS inside the platform, just-prs is a self-contained, separable component: it can equally be used on its own as a Python library, a command-line tool, an MCP server and agent skill (Section 4.2.1), or run as a lightweight standalone PRS web application with its own UI — independently of the rest of Just-DNA-Lite, or embedded in other platforms. just-prs provides access to all 5,000+ scoring files in the PGS Catalog.
...
![just-prs trait view: multiple PRS models for one trait shown together as a consensus bell curve with the user's percentile, per-model variant match rates, and quality tiers](/images/just_prs_trait_consensus.jpg)

**Figure 6. PRS results in Just-DNA-Lite (just-prs trait view).** For a selected trait, all associated PGS Catalog models are computed and shown together as a consensus distribution with the user's percentile against a 1000 Genomes reference population, alongside per-model variant match rates and quality tiers. Showing the models together — including where they disagree — is deliberate (see Sections 4.2.1 and 6.3).

### 4.2.1 Agentic Access: MCP Server, Skills, Graphics, and LLM Interpretation

Although Just-DNA-Lite presents PRS computation through its web interface, the underlying just-prs library is released as a standalone toolbox so that it can be used outside the UI entirely. This serves a distinct audience: researchers and developers who prefer to work inside AI coding assistants and agentic environments (Cursor, Codex, Antigravity, Claude Code, and similar) rather than a point-and-click application. just-prs exposes the same functionality through three complementary access paths -- a command-line interface and Python API, a Model Context Protocol (MCP) server, and an agent skill -- all backed by the identical DuckDB/Polars scoring engine described above, so results are numerically identical regardless of the entry point.

**MCP server.** just-prs ships an MCP server (published as `just-prs-mcp`) that any MCP-capable assistant can launch without cloning the repository -- for example, `claude mcp add just-prs -- uvx just-prs-mcp@latest stdio` for Claude Code, or the equivalent server entry in the configuration files used by Cursor, Codex, and Antigravity. The server exposes the toolbox as typed tool schemas, so an agent can chain the steps of a PRS workflow autonomously: download or load a genome, normalize the VCF, search the PGS Catalog by trait or gene, compute scores, and place a result within a reference-population distribution. A user can therefore issue a single plain-language request such as "normalize this genome and compute the polygenic risk score for type 2 diabetes" and have the agent orchestrate the full pipeline through structured tool calls.

**Agent skill.** For interactive use, just-prs also provides a lightweight Claude/Gemini skill (a `/prs` slash command for Claude Code and Antigravity) that teaches the assistant to call the CLI and Python API directly, without running a separate MCP server. The skill is the convenient path for quick, conversational sessions; the MCP server is preferred when an agent needs programmatic, typed tool invocation as part of a larger multi-step pipeline. Both paths target the same operations and require no pre-installation, since commands run through `uvx`.

**Explanatory graphics.** The toolbox includes built-in Altair-based visualization that renders the same chart types surfaced in the web interface: single-model bell curves with the user's score marked, multi-ancestry overlays showing all five 1000 Genomes superpopulation reference distributions, trait-grouped scatter plots of per-model z-scores coloured by quality tier, and horizontal percentile strip charts with risk-coloured bands. Charts export as interactive HTML (with hover tooltips) and Vega-Lite JSON out of the box, and as static PNG/SVG with an optional dependency. An agent can thus generate a publication-quality figure for a computed score directly in the conversation, giving non-specialist users an immediately interpretable visual rather than a bare number.

**Pre-generated interpretation prompts.** Because a PRS value is difficult to interpret in isolation, just-prs assembles a structured natural-language prompt for each result that consolidates the trait, percentile and the method used to compute it, raw score, model coverage (the fraction of the model's markers found in the user's genome, with a breakdown of observed, inferred, and unscorable variants), quality tier and discrimination metrics, absolute-risk context where available, ancestry, heritability, and source-publication links. This prompt is the same one that drives the plain-language explanations in the CLI reports and the web UI, and it can be handed to any large language model (Claude, Gemini, GPT, or others) to produce a careful, caveat-aware explanation aimed at a citizen scientist. Crucially, the prompt foregrounds the limitations discussed in Section 6.1 -- low model coverage, liftover uncertainty for harmonized scores, and the European-cohort bias of most scoring files -- so that automated explanations communicate uncertainty rather than false precision.
...
Across engines, roughly 50--54% of each scoring file's variants were matched in this single personal genome. This is expected rather than anomalous: the unmatched fraction is dominated by variants that are absent from (or not called in) this particular genome, together with indels, multiallelic sites, and strand/allele-orientation and build-coordinate differences between scoring files and the VCF. Because all engines were run on the same VCF, the match rate is comparable across them and does not affect the concordance comparison; for an individual score, a low match rate simply means fewer of the model's variants were available, and should be read as reduced coverage rather than elevated or lowered risk.
...
Just-DNA-Lite is a research tool, not a clinical pipeline. Raw VCFs contain false positives, and automated annotations have inherent error rates. The platform joins the user's variants against published databases and annotation modules and computes polygenic risk scores, adding contextual information from those sources; it does not interpret the results in a clinical context.
...
### 6.3 Platform Philosophy: Transparency, the Right to Read Your Own Genome, and the Limits of Gatekeeping

Just-DNA-Lite is built on a simple conviction: people have the right to read and explore their own genomic data, and researchers have the right to work with public genomes without a gatekeeper's permission. This is not a fringe position. The GDPR's right of access (Article 15) already entitles individuals to obtain a copy of their personal data, and the emerging European Health Data Space (EHDS) is explicitly designed to give people access to and control over their own health information. A tool that runs locally and shows you everything in your own genome is, in this light, an instrument of an existing right rather than a novelty. The platform exposes the full stack -- all modules, all 5,000+ PGS Catalog scoring files, the complete variant table cross-referenced against Ensembl -- as a research and educational resource; understanding the limitations described in Section 6.1 is essential for any downstream use.

The opposite philosophy -- tight gatekeeping, in which genomic information may reach an individual only through an approved intermediary -- is widespread and not unmotivated. Its defenders point to real risks: misinterpretation, anxiety, false reassurance, and the historical fact that some direct-to-consumer claims outran their evidence -- independent testing has found, for example, that roughly 40% of variants flagged in direct-to-consumer raw data were false positives on clinical confirmation, with most of the errors in cancer-related genes (Tandy-Connor et al., 2018). Misclassification can cause direct harm too: several cardiomyopathy variants once reported as pathogenic -- overrepresented in people of African ancestry because early control sets were predominantly white -- were later shown to be benign, after patients had already undergone years of at-risk monitoring (Manrai et al., 2016). The 2013 case of 23andMe is the canonical example: although the company framed its service as wellness and personal information, the U.S. FDA ordered it to halt marketing of its health-related reports until they were analytically and clinically validated and authorized (Annas & Elias, 2014).

We take this concern seriously, but we would argue that our approach differs in kind from what drew that regulatory action. 23andMe was a commercial provider, and despite its wellness framing, many consumers reasonably came away believing they had received actionable health findings -- a single curated result is easy to over-read, and a commercial product has every incentive to present one. Just-DNA-Lite is oriented the other way. It is not selling a reassuring answer; it surfaces the contradictions and low quality in the underlying science from the outset (discussed further below); and it pairs results with a detailed FAQ that walks through concrete examples of misinterpretation, including documented cases where a single variant or a polygenic score misled people. In other words, the very risk that gatekeeping fears -- misinterpretation and false reassurance -- is what our design attacks head-on: not by withholding information, but by making its uncertainty unavoidable and teaching the user how to read it. For us, "research use" is not a disclaimer bolted onto a clean consumer report; it is the actual texture of what the tool shows.

We are also skeptical that prohibition is the right remedy in any case, and we are not alone: it has been argued directly that the 2013 FDA clampdown on consumer genomics was not backed by evidence of actual harm and curtailed individuals' right to their own information (Green & Farahany, 2014). The harms most often invoked are also not strongly borne out empirically — a prospective study of consumer genome-wide risk profiling found no significant increase in anxiety or unhealthy behaviour among recipients (Bloss et al., 2011). A bare "wellness" label did not shield 23andMe, and outright bans fare no better. Some jurisdictions go much further than the FDA did: France not only bans direct-to-consumer genetic testing but criminalizes ordering a kit, with fines of up to €3,750 for residents who do so -- yet in practice no one is fined, and French residents simply order kits from abroad ([STAT News, 2019](https://www.statnews.com/2019/11/14/france-consumer-genetic-testing-ban/)). Where the law has instead chosen openness, access has tended to widen rather than collapse: when the U.S. Supreme Court invalidated Myriad's patents on the BRCA genes, competing tests appeared within days and prices fell (Association for Molecular Pathology v. Myriad Genetics, 2013); and 23andMe itself was eventually authorized under a regulatory framework rather than prohibited. Gatekeeping of the prohibitive kind rarely stops the determined citizen; it mainly raises the barrier for the cautious one, pushes activity offshore and out of sight, and chills exactly the kind of open, auditable tooling that would let people interpret results responsibly. In our view it suppresses innovation without delivering the protection it promises: patients and users are themselves a documented source of medical innovation (Oliveira et al., 2015), so the cost of locking them out of their own data and tools is real, not hypothetical. It also sits uneasily with the right-of-access principles above.

A related question is how to handle the noisiness of the science itself. For many popular traits the published evidence is genuinely contradictory: a single trait can have a dozen polygenic scores that place the same person at very different percentiles. Two responses are possible. One is to curate aggressively -- pick a single "best" model per trait, hide the rest, and present one clean number. This is reassuringly simple, but it conceals the disagreement, makes an editorial judgment on the user's behalf, and can simply be wrong. The other, which we adopt, is to show everything and equip the user to judge: every available model for a trait, alongside quality tiers, discrimination metrics, variant match-rates, ancestry context, and a consensus view that marks where models agree and where they scatter. This is less tidy, but it is honest about the state of the evidence and treats the user as capable of weighing it. We believe honesty about uncertainty is more respectful -- and ultimately safer -- than a false clean answer.

Transparency only helps if people can learn to use the tool, so education is part of the project rather than an afterthought. We maintain a YouTube channel of tutorials ([youtube.com/@just-dna-seq](https://www.youtube.com/@just-dna-seq)) and have run several hands-on bioinformatics workshops introducing students, researchers, and citizen scientists to local, privacy-preserving genome exploration. Beyond disseminating the software, these activities are early evidence that non-specialists can be brought up to speed on reading their own genomic data responsibly -- which is, in the end, the point of building an open and transparent tool rather than a closed one.
...
**Ancestry selection.** Automatic ancestry inference from the VCF is not yet implemented; users select the reference superpopulation closest to their ancestry, and the percentile is computed against that 1000 Genomes population. Automated ancestry estimation is planned.

**Scope.** Just-DNA-Lite is a research and educational tool. It annotates the user's VCF against published variant-trait databases and annotation modules and computes polygenic risk scores. The platform itself does not perform clinical interpretation, generate diagnoses, or provide medical recommendations; its optional, user-initiated AI explanation of results is general and non-prescriptive, not clinical advice.

**Input format.** The platform supports VCF files from whole genome sequencing (WGS) and whole exome sequencing (WES). GRCh38 is the primary build; GRCh37/hg19 inputs are now handled via coordinate liftover to GRCh38. Consumer microarray data (23andMe, AncestryDNA) is supported experimentally, but arrays read only a few hundred thousand pre-selected positions versus the millions in a sequencing VCF, so coverage is far lower and many scoring-model and module variants will be absent; the platform surfaces this limitation to users rather than masking it. T2T reference genomes are not yet supported.
...
Several extensions are already underway or planned. On the input side, GRCh37/hg19 files are now handled through coordinate liftover to GRCh38, broadening compatibility with older sequencing pipelines, and experimental support for consumer microarray data (23andMe, AncestryDNA) has been added -- though, as noted in Section 6.4, arrays interrogate far fewer positions than sequencing, and the platform is explicit with users about this inferiority. Looking further ahead, T2T (Telomere-to-Telomere) reference genome support will enable annotation of variants in regions that were previously unresolvable.

Multi-species support is a particularly compelling direction. Companion-animal genomics is advancing quickly, and the recent case of an owner with no medical background who sequenced his dog's tumour and used general-purpose AI tools to help design a personalized mRNA vaccine that reportedly drove the cancer into substantial regression (Kim, 2026) captures both the promise of accessible genomics for dogs and cats and -- as a single, non-peer-reviewed anecdote -- the very need for caution and validation that this platform emphasises.

We are also considering methylation and transcriptomic ageing clocks, which are among the analyses users most frequently ask to see and a natural extension of the platform's longevity focus.

Within the ROGEN consortium, work is underway to develop population-calibrated polygenic risk scores and to apply the platform to the 5,000-individual Romanian genomic cohort.

Beyond these technical extensions, the larger experiment is a social one. We actively encourage the community to build and share their own annotation modules, and -- by design -- we do not vet or control what others publish. This openness is the point, but it has an unavoidable consequence: community- and AI-generated modules will contain mistakes. The platform mitigates this not by gatekeeping but by making provenance visible (the `curator` field, the explicit expert-versus-AI distinction) so that trust can be calibrated rather than assumed. We regard this as a necessary experiment. The right way to curate and validate a large, decentralized ecosystem of genomic annotations -- what should be reviewed, by whom, to what standard, and how quality should be signaled to users -- is not yet known, and it will not be discovered by theorizing alone. Only by letting an ecosystem grow, watching where it succeeds and fails, and iterating on the safeguards will the field learn what a future "genomic app store" should require before a module reaches a user. Just-DNA-Lite is offered both as a usable platform and as a testbed for answering that question.

---

## 7. Conclusion

Just-DNA-Lite delivers on a simple core value: people should be able to explore their own genomes -- transparently, privately, and using whatever interface suits them. The platform offers two complementary ways to do this. For those who prefer a graphical tool, it is a self-contained local web application that annotates a whole genome and computes polygenic risk scores in tens of seconds. For those who work inside AI coding assistants, the same well-benchmarked operations are exposed through the Model Context Protocol and agent skills, so a researcher or an individual can normalize a genome, compute scores, render explanatory charts, and have results explained in plain language without leaving their assistant. The same no-code module system that lets anyone turn a YAML file and a variant table into an annotation module also serves as a fast way to prototype new agentic modules, and the AI-assisted creation pipeline turns a research paper into a validated draft in minutes, so the catalogue can grow with the literature rather than waiting on a central team.

These capabilities rest on a foundation that matches the project's philosophy. All computation is local and the code is open-source (AGPL v3), so privacy is structural rather than promised; the platform shows the full picture -- every model and every one of the 5,000+ available polygenic scores, with their quality signals and their disagreements -- rather than a curated verdict; and the performance holds up, with whole-genome annotation in tens of seconds and PRS computation validated at Pearson r = 0.9999 against PLINK2 while running several times faster and using a fraction of the memory. Just-DNA-Lite is a research and educational tool, not a clinical pipeline, and it is explicit about that boundary -- but within it, it gives researchers and individuals a fast, transparent, and extensible way to read their own genomes, by hand or through an AI assistant.
...
Annas, G. J., & Elias, S. (2014). 23andMe and the FDA. *New England Journal of Medicine*, 370(11), 985-988. [https://doi.org/10.1056/NEJMp1316367](https://doi.org/10.1056/NEJMp1316367)

Association for Molecular Pathology v. Myriad Genetics, Inc., 569 U.S. 576 (2013). U.S. Supreme Court. [https://supreme.justia.com/cases/federal/us/569/576/](https://supreme.justia.com/cases/federal/us/569/576/)

Bloss, C. S., Schork, N. J., & Topol, E. J. (2011). Effect of direct-to-consumer genomewide profiling to assess disease risk. *New England Journal of Medicine*, 364(6), 524-534. [https://doi.org/10.1056/NEJMoa1011893](https://doi.org/10.1056/NEJMoa1011893)
...
Green, R. C., & Farahany, N. A. (2014). Regulation: The FDA is overcautious on consumer genomics. *Nature*, 505(7483), 286-287. [https://doi.org/10.1038/505286a](https://doi.org/10.1038/505286a)

Kim, S. (2026). Owner with no medical background invents cure for dog's terminal cancer. *Newsweek*, March 16, 2026. [https://www.newsweek.com/owner-no-medical-background-invents-cure-dogs-terminal-cancer-11684882](https://www.newsweek.com/owner-no-medical-background-invents-cure-dogs-terminal-cancer-11684882)

Kulaga, A., et al. (2024). Just-DNA-Seq, open-source personal genomics platform: longevity science for everyone. *arXiv* preprint. [arXiv:2403.19087](https://arxiv.org/abs/2403.19087)
...
Manrai, A. K., et al. (2016). Genetic misdiagnoses and the potential for health disparities. *New England Journal of Medicine*, 375(7), 655-665. [https://doi.org/10.1056/NEJMsa1507092](https://doi.org/10.1056/NEJMsa1507092)

Oliveira, P., Zejnilović, L., Canhão, H., & von Hippel, E. (2015). Innovation by patients with rare diseases and chronic needs. *Orphanet Journal of Rare Diseases*, 10, 41. [https://doi.org/10.1186/s13023-015-0257-2](https://doi.org/10.1186/s13023-015-0257-2)
...
STAT News. (2019). In France, it's illegal for consumers to order a DNA spit kit. Activists are fighting over lifting the ban. November 14, 2019. [https://www.statnews.com/2019/11/14/france-consumer-genetic-testing-ban/](https://www.statnews.com/2019/11/14/france-consumer-genetic-testing-ban/)

Tacutu, R., et al. (2017). Human Ageing Genomic Resources: New and updated databases. *Nucleic Acids Research*, 46(D1), D1083-D1090.

Tandy-Connor, S., et al. (2018). False-positive results released by direct-to-consumer genetic tests highlight the importance of clinical confirmation testing for appropriate patient care. *Genetics in Medicine*, 20(12), 1515-1521. [https://doi.org/10.1038/gim.2018.38](https://doi.org/10.1038/gim.2018.38)
